  ...
  async function handleSubmit(e) {
    ...
    if (
      ...
      if (!inputs?.image) {
        ...
        toast.custom((t) => (
          <CustomToast t={t} message={'صورة الوصفة مطلوبة 😐'} />
        ));
        dispatch({
          type: 'IMAGE_ERROR',
          payload: { imageError: true, message: 'صورة الوصفة مطلوبة' },
        });
      } else if (!inputs?.mealName) {
        setErrors({ ...errors, mealName: true });

        toast.custom((t) => (
          <CustomToast t={t} message={'اسم الوصفة مطلوب 😐'} />
        ));
      ...
      <div className="w-full p-2 sm:p-8 h-fit ">
        ...
        >
            ...
            <div className="flex flex-col gap-8 md:flex-row w-full ">
              <div className="flex flex-col items-center justify-center w-full">
                {errors.selectedValue && (
                  <h1 className="text-primary text-2xl text-start my-2 w-full animate-bounce sm:font-bold opacity-0">
                    اختيار الصنف مطلوب
                  </h1>
                )}
                {errors.mealName && (
                  <h1 className="text-primary text-2xl text-start my-2 w-full animate-bounce sm:font-bold">
                    اسم الوصفة مطلوب
                  </h1>
                )}
                <div className="flex items-center gap-2 w-full justify-start">
                  <h1 className="text-right text-md sm:text-xl  sm:font-bold my-2 ">
                    <span className="text-primary sm:font-bold text-2xl ml-2">
                      #
                    </span>
                    اسم الوصفة: (إجباري)
                  </h1>
                ...
                <input
                  ...
                  autoFocus
                  className="text-right w-full p-2 text-lg outline-2 focus:outline-one h-10 placeholder:text-sm placeholder:sm:text-lg border border-gray-300 focus:outline-primary rounded-[5px]"
                />
              </div>
              <div className="flex flex-col items-center justify-center w-full my-2 ">
                ...
                <div className="relative w-full h-8 sm:h-14 md:hidden">
                  <Image
                    priority
                    src={'/photo (3).png'}
                    layout="fill"
          ...
          <div className="w-full my-4">
            <div className="relative w-full h-8 sm:h-14 md:h-20 lg:h-28">
              <Image
                priority
                src={'/photo (4).png'}
                layout="fill"
            ...
            <textarea
              ...
              name="المقادير"
              id="المقادير"
              className="scrollBar text-right w-full p-2 text-xl placeholder:text-sm placeholder:sm:text-lg h-36 outline-2 focus:outline-one border border-gray-300 focus:outline-primary rounded-[5px]"
...
العصير
                `}
              className="text-right w-full p-2 text-xl placeholder:text-sm placeholder:sm:text-lg h-36 outline-2 focus:outline-one border border-gray-300 focus:outline-primary rounded-[5px]"
            ></textarea>
          </div>
          <div className="w-full my-4">
            <div className="relative w-full h-8 sm:h-14 md:h-20 lg:h-28">
            ...
            <textarea
              ...
              name="المقادير"
              id="المقادير"
              placeholder={`اكتب بعض النصائح عن تحضير هذه الوصفة
                `}
              className="text-right w-full p-2 text-xl placeholder:text-sm placeholder:sm:text-lg h-24 outline-2 focus:outline-one border border-gray-300 focus:outline-primary rounded-[5px]"
            ></textarea>
          </div>
          <div className="w-full">
            <div className="relative w-full h-8 sm:h-14 md:h-20 lg:h-28">
            ...
            <div className="flex items-center gap-2 w-full justify-start ">
              ...
              <h1 className="text-right text-md sm:text-xl  sm:font-bold my-2">
                ...
                أضف رابط الوصفة من يوتيوب أو تيك توك (إجباري)
              </h1>
            </div>
            ...
            <input
              ...
              className="text-right mt-4 mb-8 w-full p-2 rounded-[5px] text-lg outline-2 focus:outline-one h-10 border border-gray-300 focus:outline-primary placeholder:text-sm placeholder:sm:text-lg"
            />
            {inputs?.link && (
              <div>
                <iframe
          ...
          <div className="flex flex-col sm:flex-row justify-around items-center gap-8 w-full my-12">
            <button
              type="submit"
              className="flex items-center justify-center text-white bg-gradient-to-r from-primary to-secondary border shadow-lg py-3 my-2 sm:text-lg sm:p-2 text-sm p-1 px-2 text-nowrap bg-five select-none rounded-full w-full max-h-12 hover:text-white hover:bg-white/20 hover:scale-[101%] transition-all ease-in-out duration-200"
            >
              نشر
            </button>
            {cancel && (
              <button
                type="text"
                className="btn bg-five rounded-lg  shadow-lg hover:outline  outline-one text-xl hover:sm:font-bold py-2 px-16 w-full"
                onClick={() => {
                  setIsVisible(false);
                  setInputs({
                    mealName: '',
