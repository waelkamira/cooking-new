...
export default function TurkishCartoon({ image }) {
  ...
  const [turkishCartoonSliderRef, turkishCartoonInstanceRef] = useKeenSlider({
    loop: false,
    mode: 'free',
  ...
  useEffect(() => {
    if (turkishCartoonInstanceRef.current) {
      turkishCartoonInstanceRef.current.update();
    }
  }, [turkishCartoon, newSeries]);

  async function fetchTurkishCartoon() {
    try {
      const response = await fetch(`/api/turkishCartoon?page=${page}&limit=4`);
      const json = await response.json();
      if (response.ok) {
        // console.log('turkishCartoon', json);

        const existingIds = new Set(
          turkishCartoon.map((episode) => episode.id)
        );
        const newTurkishCartoon = json.filter(
          (episode) => !existingIds.has(episode.id)
        );

        if (newTurkishCartoon.length > 0) {
          setTurkishCartoon((prevTurkishCartoon) => [
            ...prevTurkishCartoon,
            ...newTurkishCartoon,
          ]);
        }
      ...
      console.error('Error fetching turkishCartoon:', error);
    }
  }
  async function handleAdd(id) {
    // console.log('id', id);
    const response = await fetch('/api/turkishCartoon', {
      method: 'PUT',
  ...
  const handleEpisodeClick = (episodeName) => {
    // احفظ المسار السابق
          ...
          turkishCartoon?.map((episode) => (
            ...
            >
              ...
              <div
                ...
                key={episode?.id}
                onClick={() => handleEpisodeClick(episode?.episodeName)}
              >
                <div
                  className={
