...
export default function ActionPlanet() {
  ...
  useEffect(() => {
    if (actionInstanceRef.current) {
      actionInstanceRef.current.update();
    }
  }, [action, newSeries]);
  ...
  async function fetchAction() {
    try {
      const response = await fetch(
        `/api/serieses?page=${page}&planetName=أكشن&limit=4`
      );
      const json = await response.json();
      if (response.ok) {
        // console.log('action', action);

        const existingIds = new Set(action.map((series) => series.id));
        const newAction = json.filter((series) => !existingIds.has(series.id));

        if (newAction.length > 0) {
          setAction((prevAction) => [...prevAction, ...newAction]);
        }
      }
    } catch (error) {
